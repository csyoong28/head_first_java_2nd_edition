...
public class Java8StreamApiUnitTest {
    ...
    public static void main(String[] args) {
        Java8StreamApiUnitTest Java8StreamApiUnitTest = new Java8StreamApiUnitTest();
        Java8StreamApiUnitTest.checkOrder_whenChangeQuantityOfMethodCalls_thenCorrect();
    }
    ...
    public void checkOrder_whenChangeQuantityOfMethodCalls_thenCorrect() {
        ...
        counter = 0;
        Stream<String> stringStream = list.stream().skip(2).map(element -> {
            wasCalled();
            return element.substring(0, 3);
        });
        System.out.println(counter);    //0, the map is not called, because there is no terminal operation
    ...
    public void createStream_whenCreated_thenCorrect() {
        ...
        //assertEquals(40 <= streamIterated.findAny().get());
    }
    ...
    public void collecting_whenAsExpected_thenCorrect() {
        ...
        //assertEquals(collectorCollection instanceof List);
        //assertEquals(5, collectorCollection.size());

        String listToString = productList.stream().map(Product::getName).collect(Collectors.joining(", ", "[", "]"));

        //assertEquals(listToString.contains(",") && listToString.contains("[") && listToString.contains("]"));

        double averagePrice = productList.stream().collect(Collectors.averagingInt(Product::getPrice));
        //assertEquals(17.2 == averagePrice);
    ...
    public void customCollector_whenResultContainsAllElementsFrSource_thenCorrect() {
        ...
        ////assertEquals(linkedListOfPersons.containsAll(productList));
    }
    ...
    public void parallelStream_whenWorks_thenCorrect() {
        ...
        //assertEquals(isParallel && haveBigPrice);
    }
